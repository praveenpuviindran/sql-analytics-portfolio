# Triggers and Events
# Trigger: a block of code that executes automatically when an event takes place on a specific table
...
CREATE TRIGGER employee_insert
	AFTER INSERT ON employee_salary # specify what event needs to take place in order for this to be triggered
    FOR EACH ROW # the trigger is going to get activated for each row that is inserted
BEGIN # what's going to happen after the trigger event takes place
	INSERT INTO employee_demographics (employee_id, first_name, last_name) # insert these parameters into the trigger
    VALUES (NEW.employee_id, NEW,first_name, NEW.last_name); # only taking the new rows that were inserted instead of repeating existing rows
END $$
DELIMITER ;
